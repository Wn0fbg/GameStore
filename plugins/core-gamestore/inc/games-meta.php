...
function add_gamestore_tab_content() {
    ...
    echo '<div 
            ...
            echo '<p class="form-field">
                    <strong>
                        <label>'.__('Select Platforms', 'core-gamestore').'</label>
                    </strong>';
...
function save_gamestore_tab_fields($post_id) {
    if (isset($_POST['_gamestore_publisher'])) {
        update_post_meta(
            $post_id, 
            '_gamestore_publisher', 
            sanitize_text_field($_POST['_gamestore_publisher']));
    } else {
    }

    $platforms = array('Xbox', 'PC', 'PlayStation');
    foreach($platforms as $platform) {
        $checkbox_value = isset(
            $_POST['_platform_'.strtolower($platform)]) ? 'yes' :'no';
        update_post_meta(
            $post_id, 
